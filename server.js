...
app.get('/coletando_dados_motores/:coletaID', async (req, res) =>{
    ...
    const client = await pool.connect();
    try {
        console.log('Conectando ao Banco de Dados challenge_reply_db para inserir dados dos sensores dos motores');
     ...
     rows = consulta.rows;
     console.log('Dados do Motores Coletados com Sucesso do banco de dados challenge_reply_db');
     res.status(200).json(rows);
    } catch (err) {
        console.error('Erro ao coletar dados dos Sensores dos motores no banco de dados challenge_reply_db');
        res.status(500).send('Erro ao Coletar dados dos Sensores dos Motores do banco de dados challenge_reply_db.', err);
    } finally{
        client.release();
        console.log('Conexão com Banco de Dados challenge_reply_db liberada');
    }
